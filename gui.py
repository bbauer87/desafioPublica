...
class Gui(tk.Tk):
    '''
    Classe que atua como visão quando o programa é inicializado com interface gráfica.
    '''

    def __init__(self, *args, **kwargs):
        '''
        Inicializador responsável com criar o quadro inicial bem como as demais páginas
        do programa.        
        '''
        ...
        if self.escolha != "SAIR":
            ...
            menu_arquivo.add_command(label="Adicionar um jogo", command=self.add_jogo)
            menu_arquivo.add_separator()
            
            menu_arquivo.add_command(label="Alterar/Criar temporada", command=self.janela_temporada)
            menu_arquivo.add_command(label="Deletar temporada", command=self.del_temporada)
            menu_arquivo.add_separator()
            
            menu_arquivo.add_command(label="Alterar/Criar banco de dados", command=self.janela_banco)
            menu_arquivo.add_command(label="Deletar banco de dados", command=self.del_BD)
            menu_arquivo.add_separator()
            ...
            menu_estats.add_command(label="Consultar gráfico", command=lambda: self.ativa_frame("PaginaGrafico"))
            menu_estats.add_command(label="Consultar tabelas", command=lambda: self.ativa_frame("PaginaTabelas"))
    ...
    def inicio(self, tipo):
        '''
        Método que inicializa um banco de dados e uma temporada.

        Altera as variáveis globais NOME_BANCO e NOME_TABELA, responsáveis
        pelos nomes do BD em uso e da temporada ativa, respectivamente.
        
        Parâmetro:
        ----------
        tipo : str
            Para que o método ou crie um BD visto que não foi localizado nenhum ou então
            que liste os BDs encontrados.
        '''
    ...
    def fim(self):
        '''
        Método que diz para o controlador encerrar o cursor e a conexão com o BD.
        '''
    ...
    def atualiza_nomes(self):
        '''
        Método que atualiza a cada cinco segundos o nome do BD ativo no título da janela.
        '''
    ...
    def janela_temporada(self):
        ...
        self.botao_rad_1 = ttk.Radiobutton(janela, text="Temporadas localizadas", variable=self.botao_escolhido, value="localizar", command=lambda: escolhas("localizar"))
        self.botao_rad_1.grid(row=1, column=0, pady=10)
        
        self.botao_rad_2 = ttk.Radiobutton(janela, text="Criar temporada", variable=self.botao_escolhido, value="criar", command=lambda: escolhas("criar"))
    ...
    def sobre(self):
        '''
        Método que imprime na tela informações acerca do programa desenvolvido.
        '''
...
---------------------------------------------------------------
    ...
    def janela_banco(self):
        ...
        def confirmar():
            ...
            if self.botao_escolhido.get() == "criar":
                ...
                try:
                    ...
                    validacao = True
                    tk.messagebox.showinfo(title="Sucesso!!",  message=f"Criado BD com nome {self.entrada1.get()} e temporada {self.entrada2.get()}!")
                ...
                try:
                    ...
                    validacao = True
                    tk.messagebox.showinfo(title="Sucesso!!",  message=f"Alterado o BD para {self.listagem.get(self.listagem.curselection())}!")
        ...
        self.botao_rad_1 = ttk.Radiobutton(janela, text="Escolher BD", variable=self.botao_escolhido, value="localizar", command=lambda: escolhas("localizar"))
        self.botao_rad_1.grid(row=1, column=0, pady=10)
        
        botao_rad_2 = ttk.Radiobutton(janela, text="Criar BD", variable=self.botao_escolhido, value="criar", command=lambda: escolhas("criar"))
    ...
    def ativa_frame(self, pagina):
        '''
        Método que altera entre as páginas disponíveis.
        
        Parâmetro:
        ----------
        pagina : str
            Indica o nome da página a ser exibida.
        '''
    ...
    def del_BD(self):
        '''
        Método responsável pela deleção de BD.

        Caso haja apenas um BD no diretório do programa, impede o usuário de prosseguir.
        '''
    ...
    def del_temporada(self):
        '''
        Método responsável pela deleção de temporada.

        Caso haja apenas uma temporada no BD, impede o usuário de prosseguir.
        '''
    ...
    def add_jogo(self):
        '''
        Método responsável pela adição de um jogo na tabela.
        '''        
...
class PaginaTutorial(tk.Frame):
    '''
    Classe que constrói a janela dos tutoriais.
    '''
    ...
    def __init__(self, parent, controller):
        ...
        controller : object
            A instância da classe Gui, que controla a interface gráfica do programa.
        '''
    ...
    def imprime(self, opcao):
        '''
        Método que imprime na tela os textos de ajuda.
        
        Parâmetro:
        ----------
        opcao : str
            Define qual mensagem aparecerá na tela com base no botão clicado.
        '''
...
class PaginaGrafico(tk.Frame):
    '''
    Classe que constrói a janela do gráfico.
    '''
    ...
    def __init__(self, parent, controller):
        ...
        button1 = tk.Button(container1, text="Página inicial", command=lambda: controller.ativa_frame("PaginaInicial"), bg="brown", fg="white")
        button1.pack(padx=7, pady=7)

        self.button2 = tk.Button(container2, text="Gerar gráfico", command=lambda: self.gera(controller.controlador.retorna_dados()[0]), bg="green", fg="white")
        self.button2.pack(padx=7, pady=7)


    def gera(self, dados):
        '''
        Método que gera/atualiza o gráfico na tela, usando biblioteca matplotlib.

        Parâmetro:
        ----------
        dados : list
            Recebe os dados da tabela SQL para que possa popular o gráfico.
        '''
...
class PaginaInicial(tk.Frame):
    '''
    Classe que constrói a janela inicial do programa.
    '''
    ...
    def __init__(self, parent, controller):
        ...
        add_placar = ttk.Button(container1, text="Adicionar jogo", command=controller.add_jogo)
        add_placar.pack(side="left", padx=5, pady=5)
        
        tabelas = ttk.Button(container1, text="Tabelas", command=lambda: controller.ativa_frame("PaginaTabelas"))
        tabelas.pack(side="right", padx=5, pady=5)

        grafico = ttk.Button(container1, text="Gráfico", command=lambda: controller.ativa_frame("PaginaGrafico"))
        grafico.pack(padx=5, pady=5)

        temporada1 = ttk.Button(container2, text="Alterar/Criar", command=controller.janela_temporada)
        temporada1.pack(padx=5, pady=5)
        
        temporada2 = ttk.Button(container2, text="Deletar", command=controller.del_temporada)
        temporada2.pack(padx=5, pady=5)

        banco1 = ttk.Button(container3, text="Alterar/Criar", command=controller.janela_banco)
        banco1.pack(padx=5, pady=5)
        
        banco2 = ttk.Button(container3, text="Deletar", command= controller.del_BD)
        banco2.pack(padx=5, pady=5)
        
        sair = tk.Button(container_botoes, text="Sair", command= controller.destroy, bg="red", fg="white", width=10)
...
class PaginaTabelas(tk.Frame):
    '''
    Classe que constrói a janela das tabelas.
    '''
    ...
    def __init__(self, parent, controller):
        ...
        button1 = ttk.Button(self, text="Menu inicial", command=lambda: controller.ativa_frame("PaginaInicial"))
        button1.pack(pady=10, padx=10, side="top")

        button2 = ttk.Button(self, text="Busca tabela", command=lambda: self.atualiza(controller.controlador.retorna_dados()))
    ...
    def atualiza(self, dados):
        '''
        Método responsável por atualizar os dados nas tabelas em tela.
        '''
...
def verif_classes(param):
    '''
    Função que cria a variável global CLASSES, usada no inicializador da classe GUI.

    Faz uma varredura nas classes criadas no script, criando um dicionário com o
    nome da classe como chave e o respectivo objeto como valor.

    Parâmetro:
    ----------
    param : função
        Recebe a função globals() como parâmetro, para que seja possível percorrer
        as classes criadas no script.
    '''
